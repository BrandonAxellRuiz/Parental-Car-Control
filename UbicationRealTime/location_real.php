    ...
    if($_POST['name']!="" && $_POST['latitud']!="" && $_POST['longitud']!=""){
    	...
    	if(count($row)>0){
    		...
    		foreach ($row as $row) {
    			...
    			$result_u = $conexionm -> prepare("UPDATE `ubication_real` SET status = :status WHERE id_ubication_real = :id_ubication_real");
    			...
    			if(count($result_u -> rowCount())>0){
    				...
    				$result_i = $conexionm -> prepare("INSERT INTO `ubication_real`(`name_tester`, `latitud`, `long`, `time_create`, `date_create`, `status`) VALUES(:name_tester, :latitud, :long, :time_create, :date_create, :status)");
    				$result_i -> execute(array(
    					
    					'name_tester' => $_POST['name'], 
    					'latitud' => $_POST['latitud'],
    					'long' => $_POST['longitud'],
    					'time_create' => date("g:i a"),
