...
const UserPrivilege = localStorage.getItem("Priv");

const d = document;

window.onload = function() {
	
	d.getElementById('UserName').innerHTML = UserName;
	d.getElementById("UserImage").setAttribute("src",UserImage);
	d.getElementById("UserEmail").innerHTML = UserEmail;

}

const crud = new Firebase('https://backhome-16b56.firebaseio.com/ParentalCarControl/Car/1');
    ...
    crud.orderByChild("name_car").on('value',function(data){
        ...
        $.each(datos, function(id,values){
				...
				if(values.status == 1){
					...
					btn_status = '<a onclick="s_car('+"'"+id+"'"+',1)" id="btn_'+id+'" style="cursor:pointer !important;"><i class="material-icons">block</i><br>Bloquear</a>';
